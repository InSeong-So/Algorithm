package algorithm.doit.chap07.practice;

public class _07_05_IntSortedSet {
	private int max; // 집합의 용량
	private int num; // 집합의 요솟수
	private int[] set; // 집합본체

	// 생성자
	public _07_05_IntSortedSet(int capacity) {
		num = 0;
		max = capacity;
		try {
	...
	// 집합 s에 복사(s ← this)
	public void copyTo(_07_05_IntSortedSet s) {
	...
	// 집합 s를 copy(this ← s)
	public void copyFrom(_07_05_IntSortedSet s) {
	...
	// 집합 s와 같은가?
	public boolean equals(_07_05_IntSortedSet s) {
	...
	// 집합 s1과 s2의 합집합을 복사
	public void unionOf(_07_05_IntSortedSet s1, _07_05_IntSortedSet s2) {
	...
	// 집합 s와 합집합
	public boolean add(_07_05_IntSortedSet s) {
		boolean flag = false;
	...
	// 집합 s와 교집합
	public boolean retain(_07_05_IntSortedSet s) {
		boolean flag = false;
		for (int i = 0; i < num; i++)
	...
	// 집합 s와 차집합
	public boolean remove(_07_05_IntSortedSet s) {
		boolean flag = false;
		for (int i = 0; i < num; i++)
	...
	// 집합 s의 부분집합인가요?
	public boolean isSubsetOf(_07_05_IntSortedSet s) {
		for (int i = 0; i < num; i++) {
			int j = 0;
			for (; j < s.num; j++)
				if (set[i] == s.set[j])
	...
	// 집합 s의 진부분집합인가요?
	public boolean isProperSubsetOf(_07_05_IntSortedSet s) {
	...
	// 집합 s1과 s2의 교집합을 복사
	public void intersectionOf(_07_05_IntSortedSet s1, _07_05_IntSortedSet s2) {
		clear();
	...
	// 집합 s1과 s2의 차집합을 복사
	public void differenceOf(_07_05_IntSortedSet s1, _07_05_IntSortedSet s2) {
		clear();
		for (int i = 0; i < s1.num; i++)
			if (!s2.contains(s1.set[i]))
				add(s1.set[i]);
